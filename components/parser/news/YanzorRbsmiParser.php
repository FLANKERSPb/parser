...
/**
 * News parser from site https://yanzori.rbsmi.ru/
 * @author jcshow
 */
class YanzorRbsmiParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'https://yanzori.rbsmi.ru';
    ...
    public static function getPostDetail(DOMElement $item): NewsPost
    {
        ...
        //Get description
        $description = $itemCrawler->filterXPath('//description')->getNode(0);
        $description = self::cleanText($description->textContent);

        /** @var NewsPost */
        $post = new NewsPost(static::class, $title, $description, $createdAt, $link, $picture);

        /** Detail page parser creation */
        $descriptionHtml = $itemCrawler->filterXPath('//yandex:full-text')->getNode(0);
