...
/**
 * News parser from site https://detchip.ru/
 * @author jcshow
 */
class DetchipParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'https://detchip.ru';

    public const IMAGE_BASE_URL = 'http://detchip.ru/';
    ...
    public static function getNewsData(): array
    {
        ...
        /** Get RSS news list */
        $curl = Helper::getCurl();
        $newsList = $curl->get(static::SITE_URL . "/index.php?format=feed&type=rss");
        if (! $newsList) {
    ...
    public static function getPostDetail(DOMElement $item): NewsPost
    {
        ...
        /** Get description */
        $descriptionHtml = $itemCrawler->filterXPath('//description')->getNode(0)->textContent;
        $crawler = new Crawler($descriptionHtml);
        $descriptionBlock = $crawler->filterXPath('//span[1]')->getNode(0);
        if (empty($descriptionBlock->textContent)) {
            $description = $title;
        } else {
        ...
        self::removeNodes($crawler, '//comment() | //br | //hr | //script');

        // parse description for texts
